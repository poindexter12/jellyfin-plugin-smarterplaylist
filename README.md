...
You can also manage them from **Dashboard → Plugins → Smarter Playlist**, which lists every definition with its status, validates it against the rule engine, and lets you create and edit definitions without touching the filesystem. Hand-editing the files directly is still fully supported; the page detects a file that changed underneath it rather than overwriting your edit.
